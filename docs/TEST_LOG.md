...
- **📐 响应式卡片布局** — 服务状态三张业务卡片放入响应式网格（宽屏三卡并排、窄屏自动堆叠），对齐 Hugo 管理面板的耐用布局；「立即同步」改为主按钮
...
- **🔐 OAuth 授权窗口体验修复** — ①移动端检测：移动端同窗口跳转授权页，授权完成后自动跳回面板；②桌面端保留独立弹窗并优化关闭；③`/oauth/callback` 成功/失败页加「返回面板」按钮 + `window.opener` 自动跳回，解决移动端回不到原窗口
